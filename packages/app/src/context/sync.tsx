...
export const { use: useSync, provider: SyncProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const fetchMessages = async (input: {
      ...
      const items = raw.filter((x) => !!x?.info?.id)
      if (raw.length > 0 && items.length === 0) {
        console.warn("[sync.fetchMessages] dropped all items by filter", {
      ...
      if (raw.length === 0) {
        console.info("[sync.fetchMessages] empty response", { sessionID: input.sessionID, limit: input.limit })
      }
