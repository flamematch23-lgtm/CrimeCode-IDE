...
export default function Layout(props: ParentProps) {
  ...
  const useUpdatePolling = () =>
    onMount(() => {
      ...
      const pollUpdate = () =>
        platform.checkUpdate!().then(({ updateAvailable, version, notes }) => {
          if (!updateAvailable) return
          if (toastId !== undefined) return

          const desc = notes
            ? `${language.t("toast.update.description", { version: version ?? "" })}\n${notes}`
            : language.t("toast.update.description", { version: version ?? "" })
            ...
            actions: [
              {
                ...
                onClick: async () => {
                  // Subscribe to download progress before starting update
                  if (platform.onUpdateProgress && platform.subscribeUpdateProgress) {
                    unsub = platform.onUpdateProgress((pct) => {
                      const bar = document.getElementById("update-progress-bar")
                      if (bar) bar.style.width = `${Math.round(pct)}%`
                      const label = document.getElementById("update-progress-label")
                      if (label) label.textContent = `${Math.round(pct)}%`
                    })
                    void platform.subscribeUpdateProgress()
                  }

                  // Replace the toast with a downloading one
                  if (toastId !== undefined) toaster.dismiss(toastId)
                  toastId = showToast({
                    persistent: true,
                    icon: "download",
                    title: language.t("toast.update.title"),
                    description: `<div style="margin-top:4px"><div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:2px"><span>Downloading...</span><span id="update-progress-label">0%</span></div><div style="height:4px;border-radius:2px;background:var(--border-base);overflow:hidden"><div id="update-progress-bar" style="height:100%;width:0%;background:var(--text-base);transition:width 150ms ease;border-radius:2px"></div></div></div>`,
                  })
                  ...
                  try {
                    ...
                    // If update fails, show original toast again
                    unsub?.()
                  }
                },
              },
  ...
  command.register("layout", () => {
    const commands: CommandOption[] = [
      ...
      },
      {
        ...
        category: language.t("command.category.view"),
        keybind: "mod+shift+b",
        disabled: platform.platform !== "desktop",
        onSelect: () => {
          if (!browserUrl()) setBrowserUrl("about:blank")
        },
      },
      {
        id: "theme.cycle",
  ...
  async function navigateToProject(directory: string | undefined) {
    if (!directory) return
    if (directory === "@liveshare") {
      const target = liveshare.state.hostSession
      const dir = base64Encode("@liveshare")
      navigateWithSidebarReset(target ? `/${dir}/session/${target}` : `/${dir}/session`)
      return
    }
  ...
  const projects = createMemo(() => {
    const list = layout.projects.list()
    if (!liveshare.state.active) return list
    const synthetic = { worktree: "@liveshare", expanded: false } as LocalProject
  ...
  const sidebarContent = (mobile?: boolean) => (
    <SidebarContent
      ...
      renderProject={(project) =>
        project.worktree === "@liveshare" ? (
          <LiveShareSidebarItem selected={() => currentDir() === "@liveshare"} mobile={mobile} />
        ) : (
