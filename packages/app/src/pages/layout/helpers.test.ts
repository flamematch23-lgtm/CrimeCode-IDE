...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("crimecode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("crimecode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("crimecode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("crimecode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("crimecode://open-project")).toBeUndefined()
    expect(parseDeepLink("crimecode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "crimecode://open-project?directory=/a",
      "crimecode://other?directory=/b",
      "crimecode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("crimecode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("crimecode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("crimecode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("crimecode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "crimecode://new-session?directory=/a",
      "crimecode://open-project?directory=/b",
      "crimecode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __CRIMECODE__: {
        deepLinks: ["crimecode://open-project?directory=/a"],
      },
    } as unknown as Window & { __CRIMECODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["crimecode://open-project?directory=/a"])
