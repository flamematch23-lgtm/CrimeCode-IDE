...
export function Titlebar() {
  ...
  createEffect(() => {
    if (!platform.onUpdateReady) return
    const cleanup = platform.onUpdateReady((info) => {
      setUpdate({ ready: true, version: info.version, busy: false })
    })
    onCleanup(cleanup)
  })

  const restart = async () => {
    if (!update.ready || update.busy) return
      ...
      >
        ...
        <Show when={update.ready}>
          <Tooltip
            placement="bottom"
            value={
              update.version ? `Update ${update.version} ready — click to restart` : "Update ready — click to restart"
            }
          >
            <Button
              variant="ghost"
              class="titlebar-icon h-6 px-2 gap-1 text-xs"
              onClick={restart}
              disabled={update.busy}
              aria-label="Restart to install update"
            >
              <Icon size="small" name="download" />
              <span>{update.busy ? "Restarting..." : "Restart"}</span>
            </Button>
          </Tooltip>
        </Show>
        <Show when={windows()}>
