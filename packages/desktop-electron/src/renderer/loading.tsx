...
// Performance marks
performance.mark("loading_screen_render_start")
...
render(() => {
  ...
  onMount(() => {
    performance.mark("loading_screen_mounted")
    setLine(0)
    setPercent(0)
  ...
  createEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (window.api?.loadingWindowComplete) {
        performance.mark("loading_window_complete_start")
        window.api.loadingWindowComplete()
        performance.mark("loading_window_complete_end")
      }
    }, 1000)
    ...
    <MetaProvider>
      <div class="w-screen h-screen bg-background-base flex items-center justify-center">
        ...
        <div class="flex flex-col items-center gap-11">
          <Splash class="h-10 w-auto opacity-15" />
