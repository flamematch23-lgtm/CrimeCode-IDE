...
export function getSidecarPath() {
  ...
  const path = app.isPackaged
    ...
    : join(root, "../../sidecar", `opencode-cli${suffix}`)
  logger.log("sidecar path resolved", { path, isPackaged: app.isPackaged })
  return path
}
...
export function spawnCommand(args: string, extraEnv: Record<string, string>) {
  logger.log("spawning command", { args })
  ...
  logger.log("executing command", { cmd, args: cmdArgs.join(" ") })
  ...
  logger.log("process spawned", { pid: child.pid })
...
function buildCommand(args: string, env: Record<string, string>) {
  ...
  if (process.platform === "win32") {
    const sidecar = getSidecarPath()
    logger.log("using Windows direct mode", { sidecar })
  ...
  logger.log("using Unix mode", { shell, command: line })
