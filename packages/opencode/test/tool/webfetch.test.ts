...
describe("tool.webfetch", () => {
  test("returns image responses as file paths", async () => {
      ...
      async () => {
        await Instance.provide({
          ...
          fn: async () => {
            ...
            expect(result.output).toContain("Image saved to:")
            expect(result.output).toContain(".png")
            // Image should be saved to file, not returned as base64
            expect(result.output.includes("data:image")).toBe(false)
          },
        })
      },
    )
