...
function getImageDir(): string {
  const dir = join(homedir(), "OpenCode", "images")
  if (!existsSync(dir)) {
  ...
  async execute(params, ctx) {
    ...
    if (isImage || isPdf) {
      ...
      if (isImage) {
        ...
        const filename = `read_${randomUUID()}${ext}`
        savedPath = join(getImageDir(), filename)
        const imageData = await Filesystem.readBytes(filepath)
        writeFileSync(savedPath, Buffer.from(imageData))
      }
      const msg = `${isImage ? "Image" : "PDF"} saved to: ${savedPath}`
      return {
        title,
        output: msg,
        metadata: {
