  ...
  function unsupportedParts(msgs: ModelMessage[], model: Provider.Model): ModelMessage[] {
    return msgs.map((msg) => {
      ...
      const filtered = msg.content.map((part) => {
        ...
        const isClipboardImage = filename?.startsWith("clipboard") || filename?.startsWith("pasted-image")
        const name = filename ? `"${filename}"` : modality
        const friendlyName = isClipboardImage ? "this clipboard image" : name
        return {
          type: "text" as const,
          text: `ERROR: Cannot read ${friendlyName} (this model does not support ${modality} input). Inform the user.`,
        }
      })
