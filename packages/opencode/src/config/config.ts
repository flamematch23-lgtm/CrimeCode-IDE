...
export namespace Config {
  ...
  function globalConfigFile() {
    const candidates = ["openworm.jsonc", "openworm.json", "opencode.jsonc", "opencode.json", "config.json"].map(
    ...
    Effect.gen(function* () {
      ...
      const loadGlobal = Effect.fnUntraced(function* () {
        let result: Info = pipe(
          ...
          mergeDeep(yield* loadFile(path.join(Global.Path.config, "openworm.json"))),
          mergeDeep(yield* loadFile(path.join(Global.Path.config, "openworm.jsonc"))),
        )
      ...
      const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
        ...
        for (const dir of unique(directories)) {
          if (dir.endsWith(".openworm") || dir.endsWith(".opencode") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["openworm.jsonc", "openworm.json", "opencode.jsonc", "opencode.json"]) {
