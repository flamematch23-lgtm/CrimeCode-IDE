...
const CLI_INSTALL_DIR: &str = ".crimecode/bin";
const CLI_BINARY_NAME: &str = "crimecode";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("crimecode-install.sh");
...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "CRIMECODE_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "CRIMECODE_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("CRIMECODE_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.crimecode/bin/crimecode\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://crimecode.ai/install | bash -s -- --version {} --no-modify-path",
            ...
            let mut env_prefix = vec![
                "CRIMECODE_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "CRIMECODE_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "CRIMECODE_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "CRIMECODE_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "CRIMECODE_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "CRIMECODE_CLIENT")
...
pub fn serve(
    ...
    let envs: Vec<(&str, String)> = vec![
        ("OPENCODE_SERVER_USERNAME", "crimecode".to_string()),
        ("OPENCODE_SERVER_PASSWORD", password.to_string()),
        ("OPENCODE_DISABLE_MODELS_FETCH", "1".to_string()),
        ("OPENCODE_ENABLE_EXPERIMENTAL_MODELS", "1".to_string()),
        ("OPENCODE_ENABLE_QUESTION_TOOL", "1".to_string()),
        ("CRIMECODE_SERVER_USERNAME", "crimecode".to_string()),
        ("CRIMECODE_SERVER_PASSWORD", password.to_string()),
        ("CRIMECODE_DISABLE_MODELS_FETCH", "1".to_string()),
        ("CRIMECODE_ENABLE_EXPERIMENTAL_MODELS", "1".to_string()),
    ];
    ...
    .expect("Failed to spawn crimecode");
...
mod tests {
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        ...
        assert_eq!(merged.get("CRIMECODE_CLIENT"), Some(&"desktop".to_string()));
    }
