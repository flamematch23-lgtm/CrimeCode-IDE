...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("crimecode-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("crimecode_lib=debug,crimecode_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("crimecode_lib=info,crimecode_desktop=info,sidecar=info")
        }
    });
