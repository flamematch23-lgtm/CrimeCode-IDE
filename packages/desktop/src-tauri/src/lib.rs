...
// ── LiveShare IPC commands ────────────────────────────────────────────────────

/// Minimal helper to call the sidecar HTTP API.
/// `base_url` must already include the auth header logic; here we just hit the
/// endpoint via reqwest through the tauri http plugin.
async fn sidecar_post(url: String, body: serde_json::Value) -> Result<serde_json::Value, String> {
...
async fn sidecar_get(url: String) -> Result<serde_json::Value, String> {
    let client = reqwest::Client::new();
...
#[tauri::command]
#[specta::specta]
async fn liveshare_start(
    state: State<'_, SidecarReady>,
    relay_url: Option<String>,
    token: Option<String>,
) -> Result<String, String> {
    let data = state.inner().0.clone().await.map_err(|_| "sidecar not ready".to_string())?;
    let base = data.url;
    let mut body = serde_json::json!({});
    if let Some(r) = relay_url {
        body["relay"] = serde_json::Value::String(r);
    }
    if let Some(t) = token {
        body["token"] = serde_json::Value::String(t);
    }
    let resp = sidecar_post(format!("{base}/liveshare/start"), body).await?;
    resp["code"]
        .as_str()
        .map(|s| s.to_string())
        .ok_or_else(|| resp["error"].as_str().unwrap_or("unknown error").to_string())
}

#[tauri::command]
#[specta::specta]
async fn liveshare_stop(state: State<'_, SidecarReady>) -> Result<(), String> {
    let data = state.inner().0.clone().await.map_err(|_| "sidecar not ready".to_string())?;
    let base = data.url;
    sidecar_post(format!("{base}/liveshare/stop"), serde_json::json!({})).await?;
    Ok(())
}

#[tauri::command]
#[specta::specta]
async fn liveshare_status(state: State<'_, SidecarReady>) -> Result<LiveShareStatus, String> {
    let data = state.inner().0.clone().await.map_err(|_| "sidecar not ready".to_string())?;
    let base = data.url;
    let resp = sidecar_get(format!("{base}/liveshare")).await?;
    let active = resp["active"].as_bool().unwrap_or(false);
    if !active {
        return Ok(LiveShareStatus { active: false, code: None, relay: None, port: None, participant_count: 0 });
    }
    let hub = &resp["hub"];
    let code = hub["code"].as_str().map(|s| s.to_string());
    let relay = hub["relay"].as_str().map(|s| s.to_string());
    let port = hub["port"].as_u64().map(|p| p as u16);
    let count = hub["participants"].as_array().map(|a| a.len() as u32).unwrap_or(0);
    Ok(LiveShareStatus { active, code, relay, port, participant_count: count })
}

#[tauri::command]
#[specta::specta]
async fn liveshare_kick(
    state: State<'_, SidecarReady>,
    id: String,
    ...
    sidecar_post(format!("{base}/liveshare/kick/{id}"), body).await?;
    Ok(())
}
...
async fn initialize(app: AppHandle) {
    ...
    let _ = ready_tx.send(ServerReadyData {
        url: url.clone(),
        username: Some("crimecode".to_string()),
    ...
    let sqlite_done = needs_migration.then(|| {
        tracing::info!(
            path = %crimecode_db_path().expect("failed to get db path").display(),
    ...
    let loading_task = tokio::spawn({
        async move {
            if let Some(sqlite_done_rx) = sqlite_done {
                match timeout(Duration::from_secs(30), sqlite_done_rx).await {
                    Ok(_) => tracing::info!("SQLite migration completed"),
                    Err(_) => {
                        let exists = sqlite_file_exists();
                        tracing::warn!(
                            db_exists = exists,
                            "Timed out waiting for SQLite migration event; continuing startup"
                        );
                    }
                }
            }
...
fn get_sidecar_port() -> u32 {
    option_env!("CRIMECODE_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("CRIMECODE_PORT").ok())
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = crimecode_db_path() else {
        return true;
    };
...
fn crimecode_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("crimecode").join("crimecode.db"))
}
