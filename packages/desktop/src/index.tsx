...
declare const __CRIMECODE_COMMIT__: string
...
const deepLinkEvent = "crimecode:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__CRIMECODE__ ??= {}
  const pending = window.__CRIMECODE__.deepLinks ?? []
  window.__CRIMECODE__.deepLinks = [...pending, ...urls]
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__CRIMECODE__?.wsl) return undefined
    return commands.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__CRIMECODE__?.wsl) return result
  ...
  return {
    ...
    os,
    version: pkg.version,
    commit: typeof __CRIMECODE_COMMIT__ !== "undefined" ? __CRIMECODE_COMMIT__ : undefined,
      ...
      await Promise.resolve()
        .then(() => {
          const notification = new Notification(title, {
            body: description ?? "",
            icon: "https://crimecode.ai/favicon-96x96-v3.png",
          })
    ...
    getWslEnabled: async () => {
      ...
      return window.__CRIMECODE__!.wsl ?? false
    },
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("crimecode.global.dat").getItem("language")
    ...
    <PlatformProvider value={platform}>
      <AppBaseProviders locale={locale.latest}>
        <Show when={!defaultServer.loading && !sidecar.loading && !locale.loading}>
          {(_) => {
            return (
              <AppInterface defaultServer={ServerConnection.Key.make("sidecar")} servers={servers()}>
                <Inner />
              </AppInterface>
            )
          }}
